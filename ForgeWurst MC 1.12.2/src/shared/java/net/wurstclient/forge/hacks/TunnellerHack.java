...
public final class TunnellerHack extends Hack {
	...
	private final SliderSetting limit = new SliderSetting("限制",
			"挖掘到指定长度后\n"
					+ "自动停止.\n\n" + "0 = 无限制",
			0, 0, 1000, 1,
			v -> v == 0 ? "禁用" : v == 1 ? "1 方块" : (int) v + " 方块");

	private final CheckboxSetting torches = new CheckboxSetting(
			"放置火把", "放置足够多的火把\n"
					+ "来阻止怪物\n" + "生成在隧道内部.",
			false);
	...
	public TunnellerHack() {
		super("挖掘隧道",
				"自动挖掘隧道.\n\n" + ChatFormatting.RED
						+ ChatFormatting.BOLD + "警告:" + ChatFormatting.RESET
						+ " 尽管这个机器人会尽量避开\n"
						+ "岩浆和其他危险, 但仍无法保证\n"
						+ "这不会导致死亡. 请只带着你\n"
						+ "不介意丢失的装备挖掘.");
	...
	private class FillInFloorTask extends Task {
		...
		public void run() {
			...
			Comparator<BlockPos> comparator = Comparator.<BlockPos>comparingDouble(
