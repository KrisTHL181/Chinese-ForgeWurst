...
public final class ItemEspHack extends Hack {
	private final CheckboxSetting names = new CheckboxSetting("显示物品名称", true);
	private final EnumSetting<Style> style = new EnumSetting<>("样式", Style.values(), Style.BOXES);
	...
	private void renderBoxes(double partialTicks) {
		for (EntityItem e : items) {
			...
			if (names.isChecked() && mc.getRenderManager().options != null) {
				// Merged (Fix NullPointerException)
				// https://github.com/Wurst-Imperium/ForgeWurst/pull/2/commits/facda828d9222a5f8a1fd9383d1f9f9b4f384875
